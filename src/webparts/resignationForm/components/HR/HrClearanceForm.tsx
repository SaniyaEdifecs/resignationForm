...
const HrClearance = (props) => {
  ...
  const formFields = [
    "Resignationemailacceptance",
    "ResignationAcceptancecomments",
    "ELBalance",
    "ELBalanceComments",
    "ExitInterview",
    "ExitInterviewComments",
    "Gratuity",
    "GratuityComments",
    "Relocation_x002f_ReferralBonus",
    "Relocation_x002f_ReferralBonusCo",
    "Sign_x002d_onBonus",
    "Sign_x002d_onBonusComments",
    "TerminateOnHRSystems",
    "TerminateOnHRSystemsComments",
    "Waiver",
    "WaiverComments",
    "MessageToAssociate",
    ...
    "Others",
    "OthersComments",
    "ReferralBonusComments",
    "ReferralBonus",
    "HomeAllowanceRecovery",
    "HomeAllowanceRecoveryComments",
  ];
  ...
  const onSubmitForm = (value) => {
    ...
    SharePointService.getListByTitle("HrClearance")
      .items.getById(ID)
      .update(payload)
      .then(
        (items) => {
    ...
    SharePointService.getListByTitle("HrClearance")
      ...
      .then(
        (response: any) => {
          ...
          SharePointService.getListByTitle("ResignationList")
            .items.getById(detail.EmployeeNameId)
            .get()
            .then((resignDetails: any) => {
    ...
    SharePointService.getCurrentUser().then((response) => {
      ...
      if (currentUser) {
        const url =
          ...
          "/_api/web/lists/getbytitle('HrClearance')/getusereffectivepermissions(@u)?@u='" +
          encodeURIComponent(currentUser.LoginName) +
          "'";
            ...
            (permissionResponse) => {
              ...
              if (statusValue == "Approved" || statusValue == "Canceled") {
                SharePointService.getCurrentUserGroups().then((groups: any) => {
                  let isGroupOwner = groups.filter(
                    (group) => group.Title === "Resignation Group - Owners"
                  ).length;
                ...
                if (
                  permissionLevel.High == 2147483647 &&
                  permissionLevel.Low == 4294705151
                ) {
                  setReadOnly(false);
                } else if (
                  permissionResponse.error ||
                  (permissionLevel.High == 176 &&
                    permissionLevel.Low == 138612833) ||
                  (permissionLevel.High == 48 &&
                    permissionLevel.Low == 134287360)
                ) {
                  console.log(
                    "permissionResponse.error:",
                    permissionResponse.error
  ...
  return (
    <div>
      ...
      <Breadcrumbs separator="›" aria-label="breadcrumb" className="marginZero">
        <Link
          color="inherit"
          onClick={() => SharePointService.redirectTo(strings.HomeUrl, "")}
        ...
        <Link
          color="inherit"
          onClick={() => SharePointService.redirectTo(strings.HrDashboard, "")}
        >
          {strings.Dashboard}
        </Link>
      ...
      {showMsg && (
        <div className={classes.root}>
          <Alert severity="warning" className="marginTop16">
            This resignation is withdrawn - No Action Required!
          </Alert>
        </div>
      )}
      <form onSubmit={handleOnSubmit} className="clearanceForm">
          ...
          <tbody>
            <tr>
              <td>
                ...
                <span>{resignationDetails["EmployeeName"]}</span>
              </td>
              <td>
                <span>Employee Code: </span>
                <span>{resignationDetails["EmployeeCode"]}</span>
              </td>
            </tr>
            <tr>
              <td>
              ...
              <td>
                ...
                <span>
                  <Moment format="DD/MM/YYYY">
                    {resignationDetails["LastWorkingDate"]}
                  </Moment>
                </span>
              </td>
            </tr>
        ...
        <table cellSpacing="0" cellPadding="0">
          ...
          <tbody>
            <tr>
              <td>
                Resignation email & acceptance<span>*</span>
              </td>
              <td>
                <FormControl>
                  <Select
                    value={state.Resignationemailacceptance.value}
                    disabled={readOnly}
                    id="Resignationemailacceptance"
                    onBlur={handleOnBlur}
                    onChange={handleOnChange}
                    name="Resignationemailacceptance"
                    autoFocus
                  >
                    {options.map((option) => (
                  ...
                  {state.Resignationemailacceptance.error && (
                    <p style={errorStyle}>
                      {state.Resignationemailacceptance.error}
                    </p>
                  )}
              ...
              <td>
                <TextField
                  margin="normal"
                  name="ResignationAcceptancecomments"
                  disabled={readOnly}
                  onBlur={handleOnBlur}
                  onChange={handleOnChange}
                  value={state.ResignationAcceptancecomments.value}
                />
                {state.ResignationAcceptancecomments.error && (
                  <p style={errorStyle}>
                    {state.ResignationAcceptancecomments.error}
                  </p>
                )}
            ...
            <tr>
              <td>
                Exit Interview<span>*</span>
              </td>
              <td>
                <FormControl>
                  <Select
                    value={state.ExitInterview.value}
                    disabled={readOnly}
                    id="ExitInterview"
                    onBlur={handleOnBlur}
                    onChange={handleOnChange}
                  ...
                  {state.ExitInterview.error && (
                    <p style={errorStyle}>{state.ExitInterview.error}</p>
                  )}
                </FormControl>
              </td>
              <td>
                <TextField
                  ...
                  onBlur={handleOnBlur}
                  onChange={handleOnChange}
                  value={state.ExitInterviewComments.value}
                />
                {state.ExitInterviewComments.error && (
                  <p style={errorStyle}>{state.ExitInterviewComments.error}</p>
                )}
              </td>
            </tr>
            <tr>
              <td>
                Relocation Bonus<span>*</span>
              </td>
              <td>
                <FormControl>
                  <Select
                    value={state.Relocation_x002f_ReferralBonus.value}
                    disabled={readOnly}
                    id="Relocation_x002f_ReferralBonus"
                    onBlur={handleOnBlur}
                    onChange={handleOnChange}
                    name="Relocation_x002f_ReferralBonus"
                  >
                    {options.map((option) => (
                      <MenuItem value={option}>{option}</MenuItem>
                    ))}
                  </Select>
                  {state.Relocation_x002f_ReferralBonus.error && (
                    <p style={errorStyle}>
                      {state.Relocation_x002f_ReferralBonus.error}
                    </p>
                  )}
              ...
              <td>
                <TextField
                  margin="normal"
                  name="Relocation_x002f_ReferralBonusCo"
                  disabled={readOnly}
                  onBlur={handleOnBlur}
                  onChange={handleOnChange}
                  value={state.Relocation_x002f_ReferralBonusCo.value}
                />
                {state.Relocation_x002f_ReferralBonusCo.error && (
                  <p style={errorStyle}>
                    {state.Relocation_x002f_ReferralBonusCo.error}
                  </p>
                )}
            ...
            <tr>
              <td>
                Referral Bonus<span>*</span>
              </td>
              <td>
                <FormControl>
                  <Select
                    value={state.ReferralBonus.value}
                    disabled={readOnly}
                    id="ReferralBonus"
                    onBlur={handleOnBlur}
                    onChange={handleOnChange}
                  ...
                  {state.ReferralBonus.error && (
                    <p style={errorStyle}>{state.ReferralBonus.error}</p>
                  )}
                </FormControl>
              </td>
              <td>
                <TextField
                  margin="normal"
                  name="ReferralBonusComments"
                  disabled={readOnly}
                  onBlur={handleOnBlur}
                  onChange={handleOnChange}
                  value={state.ReferralBonusComments.value}
                />
                {state.ReferralBonusComments.error && (
                  <p style={errorStyle}>{state.ReferralBonusComments.error}</p>
                )}
              </td>
            </tr>
            <tr>
              <td>
                Sign-on Bonus<span>*</span>
              </td>
              <td>
                <FormControl>
                  <Select
                    value={state.Sign_x002d_onBonus.value}
                    id="Sign_x002d_onBonus"
                    disabled={readOnly}
                    onBlur={handleOnBlur}
                    onChange={handleOnChange}
                    name="Sign_x002d_onBonus"
                  >
                    {options.map((option) => (
                      <MenuItem value={option}>{option}</MenuItem>
                    ))}
                  </Select>
                  {state.Sign_x002d_onBonus.error && (
                    <p style={errorStyle}>{state.Sign_x002d_onBonus.error}</p>
                  )}
                </FormControl>
              </td>
              <td>
                <TextField
                  margin="normal"
                  name="Sign_x002d_onBonusComments"
                  disabled={readOnly}
                  onBlur={handleOnBlur}
                  onChange={handleOnChange}
                  value={state.Sign_x002d_onBonusComments.value}
                />
                {state.Sign_x002d_onBonusComments.error && (
                  <p style={errorStyle}>
                    {state.Sign_x002d_onBonusComments.error}
                  </p>
                )}
            ...
            <tr>
              <td>
                EL Balance<span>*</span>
              </td>
              <td>
                <FormControl>
                  <Select
                    value={state.ELBalance.value}
                    id="ELBalance"
                    disabled={readOnly}
                    onBlur={handleOnBlur}
                    onChange={handleOnChange}
                  ...
                  {state.ELBalance.error && (
                    <p style={errorStyle}>{state.ELBalance.error}</p>
                  )}
                </FormControl>
              </td>
              <td>
                <TextField
                  margin="normal"
                  name="ELBalanceComments"
                  disabled={readOnly}
                  onBlur={handleOnBlur}
                  onChange={handleOnChange}
                  value={state.ELBalanceComments.value}
                />
                {state.ELBalanceComments.error && (
                  <p style={errorStyle}>{state.ELBalanceComments.error}</p>
                )}
              </td>
            </tr>
            {/* 
            ...
            <tr>
              <td>
                Terminate On Hr Systems<span>*</span>
              </td>
              <td>
                <FormControl>
                  <Select
                    value={state.TerminateOnHRSystems.value}
                    disabled={readOnly}
                    id="TerminateOnHRSystems"
                    onBlur={handleOnBlur}
                    onChange={handleOnChange}
                    name="TerminateOnHRSystems"
                  >
                    {options.map((option) => (
                      <MenuItem value={option}>{option}</MenuItem>
                    ))}
                  </Select>
                  {state.TerminateOnHRSystems.error && (
                    <p style={errorStyle}>{state.TerminateOnHRSystems.error}</p>
                  )}
                </FormControl>
              </td>
              <td>
                <TextField
                  margin="normal"
                  name="TerminateOnHRSystemsComments"
                  disabled={readOnly}
                  onBlur={handleOnBlur}
                  onChange={handleOnChange}
                  value={state.TerminateOnHRSystemsComments.value}
                />
                {state.TerminateOnHRSystemsComments.error && (
                  <p style={errorStyle}>
                    {state.TerminateOnHRSystemsComments.error}
                  </p>
                )}
            ...
            <tr>
              <td>
                Shortfall of Notice (Waiver if any)<span>*</span>
              </td>
              <td>
                <FormControl>
                  <Select
                    value={state.Waiver.value}
                    disabled={readOnly}
                    id="Waiver"
                    onBlur={handleOnBlur}
                    onChange={handleOnChange}
                  ...
                  {state.Waiver.error && (
                    <p style={errorStyle}>{state.Waiver.error}</p>
                  )}
                </FormControl>
              </td>
              <td>
                <TextField
                  ...
                  onBlur={handleOnBlur}
                  onChange={handleOnChange}
                  value={state.WaiverComments.value}
                />
                {state.WaiverComments.error && (
                  <p style={errorStyle}>{state.WaiverComments.error}</p>
                )}
              </td>
            </tr>
            ...
            <tr>
              ...
              <td>
                <FormControl>
                  ...
                  {state.Gratuity.error && (
                    <p style={errorStyle}>{state.Gratuity.error}</p>
                  )}
                </FormControl>
              </td>
              <td>
                <TextField
                  margin="normal"
                  name="GratuityComments"
                  disabled={readOnly}
                  onBlur={handleOnBlur}
                  onChange={handleOnChange}
                  value={state.GratuityComments.value}
                />
                {state.GratuityComments.error && (
                  <p style={errorStyle}>{state.GratuityComments.error}</p>
                )}
              </td>
            </tr>
            <tr>
              <td>
                Recovery on home office reimbursement program<span>*</span>
              </td>
              <td>
                <FormControl>
                  <Select
                    value={state.HomeAllowanceRecovery.value}
                    id="HomeAllowanceRecovery"
                    disabled={readOnly}
                    onBlur={handleOnBlur}
                    onChange={handleOnChange}
                    name="HomeAllowanceRecovery"
                  >
                    {options.map((option) => (
                      <MenuItem value={option}>{option}</MenuItem>
                    ))}
                  </Select>
                  {state.HomeAllowanceRecovery.error && (
                    <p style={errorStyle}>
                      {state.HomeAllowanceRecovery.error}
                    </p>
                  )}
              ...
              <td>
                <TextField
                  margin="normal"
                  name="HomeAllowanceRecoveryComments"
                  disabled={readOnly}
                  onBlur={handleOnBlur}
                  onChange={handleOnChange}
                  value={state.HomeAllowanceRecoveryComments.value}
                />
                {state.HomeAllowanceRecoveryComments.error && (
                  <p style={errorStyle}>
                    {state.HomeAllowanceRecoveryComments.error}
                  </p>
                )}
            ...
            <tr>
              <td>
                Others (Specify)<span>*</span>
              </td>
              <td>
                <FormControl>
                  <Select
                    value={state.Others.value}
                    id="Others"
                    disabled={readOnly}
                    onBlur={handleOnBlur}
                    onChange={handleOnChange}
                  ...
                  {state.Others.error && (
                    <p style={errorStyle}>{state.Others.error}</p>
                  )}
                </FormControl>
              </td>
              <td>
                <TextField
                  ...
                  onBlur={handleOnBlur}
                  onChange={handleOnChange}
                  value={state.OthersComments.value}
                />
                {state.OthersComments.error && (
                  <p style={errorStyle}>{state.OthersComments.error}</p>
                )}
              </td>
            </tr>
          </tbody>
        ...
        <div className="noBoxShadow ">
          <RadioGroup
            aria-label="DuesPending"
            name="DuesPending"
            value={state.DuesPending.value}
            onChange={handleOnChange}
            onBlur={handleOnChange}
          >
            <FormControlLabel
              value="NotifyAssociate"
              control={<Radio disabled={readOnly} />}
              label="Message to Associate"
            />
            ...
            {state.DuesPending.value === "NotifyAssociate" ? (
              <div>
                <TextField
                  ...
                  onChange={handleOnChange}
                  onBlur={handleOnChange}
                  value={state.MessageToAssociate.value}
                />
                {state.MessageToAssociate.error && (
                  <p style={errorStyle}>{state.MessageToAssociate.error}</p>
                )}
              </div>
            ) : (
              ""
          ...
          {state.DuesPending.error ? (
            <p style={errorStyle}>{state.DuesPending.error}</p>
          ) : (
            ""
          )}
        </div>
