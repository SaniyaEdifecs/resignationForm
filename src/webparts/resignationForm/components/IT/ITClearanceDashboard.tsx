...
const ITClearanceDashboard = () => {
    ...
    const getClearanceList = () => {
        showLoader(true);
        SharePointService.getListByTitle("ItClearance").items.select('Id', 'Status', 'EmployeeNameId', 'EmployeeName/Id', 'EmployeeName/EmployeeCode', 'EmployeeName/EmployeeName', 'EmployeeName/ManagerName').expand("EmployeeName").orderBy("Created", false).get().then((items: any) => {
            showLoader(false);
            if (items) {
    ...
    const handleClick = (event) => {
        window.location.href = "?component=itClearance&resignationId=" + event;
    };
