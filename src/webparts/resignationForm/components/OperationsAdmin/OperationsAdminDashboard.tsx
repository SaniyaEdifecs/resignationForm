...
import { withStyles, Theme, Typography, Paper, Breadcrumbs, Link, makeStyles, } from '@material-ui/core';
...
const OperationsAdminDashboard = (props) => {
    ...
    const getClearanceList = () => {
        showLoader(true);
        SharePointService.getListByTitle("OperationsClearance").items.select('Id', 'Status', 'EmployeeNameId', 'EmployeeName/EmployeeCode', 'EmployeeName/EmployeeName', 'EmployeeName/ManagerName').expand("EmployeeName").orderBy("Created", false).getAll().then((items: any) => {
            showLoader(false);
            if (items) {
    ...
    const handleClick = (event) => {
        window.location.href = "?component=operationsClearance&resignationId=" + event.rowData[0];
    };
