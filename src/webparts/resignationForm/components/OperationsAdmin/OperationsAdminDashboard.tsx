...
const OperationsAdminDashboard = (props) => {
    ...
    const getClearanceList = () => {
        showLoader(true);
        SharePointService.getListByTitle("OperationsClearance").items.select('Id', 'Status', 'EmployeeNameId', 'EmployeeName/EmployeeCode', 'EmployeeName/EmployeeName', 'EmployeeName/ManagerName').expand("EmployeeName").orderBy("Created", false).get().then((items: any) => {
            showLoader(false);
            if (items) {
