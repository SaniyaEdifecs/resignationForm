...
const EmployeeDetails = (props) => {
  ...
  const handleDateChange = (event) => {
    setState((prevState) => ({
      ...prevState,
      ["LastWorkingDate"]: { value: event, error: "" },
    }));
  };
  const handleResignationDateChange = (event) => {
    setState((prevState) => ({
      ...prevState,
      ["ResignationDate"]: { value: event, error: "" },
    }));
  };
    ...
    SharePointService.getCurrentUser().then((response) => {
      ...
      if (currentUser) {
        const url =
          ...
          "/_api/web/lists/getbytitle('Employee%20Details')/items(" +
          ID +
          ")/getusereffectivepermissions(@u)?@u='" +
      ...
      SharePointService.getListByTitle("Employee%20Details")
        ...
        .then((item) => {
          SharePointService.getListByTitle("ResignationList")
            .items.getById(employeeNameId)
            .update({
              PersonalEmail: elements.PersonalEmail,
              ResignationDate: elements.ResignationDate,
              Location: elements.Location,
              emplStatus: "Approved",
              PersonalPhone: elements.PersonalPhone,
            })
            .then((response) => {});
          if (item) {
            showLoader(false);
            ...
            window.location.href =
              "?component=resignationDetail&resignationId=" + employeeNameId;
            ...
            <Grid container spacing={2}>
              <Grid item xs={12} sm={6}>
                <TextField
                  ...
                  fullWidth
                  label="Employee Code"
                  value={state.EmployeeCode.value}
                  name="EmployeeCode"
                  autoComplete="off"
                  onChange={handleOnChange}
                  onBlur={handleOnBlur}
                  helperText="Please write code as written on pay slip"
                />
                {state.EmployeeCode.error && (
                  <p style={errorStyle}>{state.EmployeeCode.error}</p>
                )}
              </Grid>
              <Grid item xs={12} sm={6}>
                <PeoplePicker
                  context={props.context}
                  defaultSelectedUsers={[state.WorkEmail.value]}
                  ensureUser={true}
              ...
              <Grid item xs={12} sm={6}>
                <MuiPickersUtilsProvider utils={DateFnsUtils}>
                  <KeyboardDatePicker
                    ...
                    className="fullWidth"
                    format="dd-MM-yyyy"
                    value={state.LastWorkingDate.value}
              ...
              <Grid item xs={12} sm={6}>
                <MuiPickersUtilsProvider utils={DateFnsUtils}>
                  <KeyboardDatePicker
                    ...
                    className="fullWidth"
                    format="dd-MM-yyyy"
                    value={state.ResignationDate.value}
            ...
            <Grid container spacing={2}>
              <Grid item xs={12} sm={6}>
                <TextField
                  ...
                  value={state.PersonalEmail.value}
                  name="PersonalEmail"
                  onBlur={handleOnBlur}
                  autoComplete="personalEmail"
                  onChange={handleOnChange}
                />
                {state.PersonalEmail.error && (
                  <p style={errorStyle}>{state.PersonalEmail.error}</p>
                )}
              </Grid>
              <Grid item xs={12} sm={6}>
                ...
                <TextField
                  ...
                  inputProps={{ guide: false, mask, placeholderChar: "\u2000" }}
                  type="tel"
                  value={state.PersonalPhone.value}
                />
                {state.PersonalPhone.error && (
                  <p style={errorStyle}>{state.PersonalPhone.error}</p>
                )}
              </Grid>
            </Grid>
            <Grid container spacing={2}>
